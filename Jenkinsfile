    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
			   ...
			   rmsg = bat returnStdout: true, script: "${toolbelt} force:mdapi:deploy -d C:/Usama Khalid/MyStandardOrgData/MySalesforce-LocalRepo-/manifest/package.xml -u ${HUB_ORG}"
			}
